...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        // Скопируйте ваше решение из предыдущих уроков.
        return type_ == UnaryPlus ? operand_->Evaluate(sheet) : -1.0*operand_->Evaluate(sheet);
    }
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        CellInterface::Value data = cell->GetValue(); 

        if (std::holds_alternative<double>(data)) {
            return std::get<double>(data);
        } 

        if (std::holds_alternative<FormulaError>(data)) {            
            throw FormulaError(std::get<FormulaError>(data));
        }
        ...
        double result;
        std::stringstream sstrem(str);

        if (!(sstrem >> result) || !sstrem.eof()) 
